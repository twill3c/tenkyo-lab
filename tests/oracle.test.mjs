...
describe("生成器の部品", () => {
  ...
  it("T-310 バイグラムは記号をまたがない", () => {
    // loop_001 では記号を除去してから切っていたため「いう」ができていた。
    // 実在しない語で、識別力の判定では稀な語に化け、BM25 では偽の一致を生む(loop_002 で修正)
    expect([...bigrams("あい、うえ")]).toEqual(["あい", "うえ"]);
    // 陰性対照: またいだ語を作っていないこと
    expect([...bigrams("あい、うえ")]).not.toContain("いう");
    // 陽性対照: 区切りが無ければ普通に連続して切る
    expect([...bigrams("あいうえ")]).toEqual(["あい", "いう", "うえ"]);
    // 1 文字の断片は落とさず 1-gram として拾う(落とすと短い語が索引から消える)
    expect([...bigrams("あ、いう")]).toEqual(["あ", "いう"]);
  });
});
